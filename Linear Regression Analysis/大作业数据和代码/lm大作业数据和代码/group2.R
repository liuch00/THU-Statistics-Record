...
fit_i_2<-lm(Y_i~CFS_i+FCR_i+FE_i+fer_i+GDP_i+IP_i+ PS_i+SIF_i+  NADS_i)

summary(fit_i_2)

vif(fit_i_2, digits = 3)
fit_i_2.step <- step(fit_i_2, direction = "backward")
summary(fit_i_2.step)
vif(fit_i_2.step)
...
nn=C%*%rot
econ_i=nn[,1]
plot(econ_i, Y_i, pch=19, cex=.2)

E=cbind(econ_i)
 E<-as.data.frame(E)
 b=boxcox(y_i~., data=E)
 fit_2_end<-lm((y_i)^(0.50505051)~econ_i)
 summary(fit_2_end)
 ncvTest(fit_2_end)
 qqnorm(fit_2_end[["residuals"]])
