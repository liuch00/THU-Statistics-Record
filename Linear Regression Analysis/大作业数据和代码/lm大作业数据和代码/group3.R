...
fit_i_3<-lm(Y_i~CFS_i+FCR_i+FE_i+fer_i+GDP_i+IP_i+ PS_i+SIF_i+  NADS_i)

summary(fit_i_3)
vif(fit_i_3, digits = 3)

fit_i_3.step <- step(fit_i_3, direction = "backward")
summary(fit_i_3.step)
vif(fit_i_3.step)
...
end<-lm(Y_i~FE_i+econ_i+SIF_i+  NADS_i)
summary(end)
vif(end)
ncvTest(end)
