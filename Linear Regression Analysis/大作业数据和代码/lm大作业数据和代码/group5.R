...
SIF_i=0
NADS_i=0
for (i in c(8,11,13,14,18,20,21,22,24,25,27)){
...
fit_i_5<-lm(Y_i~CFS_i+FCR_i+FE_i+fer_i+GDP_i+IP_i+ PS_i+SIF_i+  NADS_i)

summary(fit_i_5)
vif(fit_i_5, digits = 3)

fit_i_5.step <- step(fit_i_5, direction = "backward")
summary(fit_i_5.step)
vif(fit_i_5.step)
...
fd_pca <- prcomp(D)
D<-as.matrix(D)
frot=fd_pca[["rotation"]]
fn=D%*%frot
fn_i=fn[,1]
end<-lm(Y_i~FCR_i+fn_i+ econ_i+SIF_i)

summary(end)
end.step <- step(end, direction = "backward")
summary(end.step)
vif(end.step)
ncvTest(end.step)
qqnorm(end.step[["residuals"]])